...
# Promotes the aarch64-linux chef-test-kitchen-enterprise package between
...
#   pkg_idents["chef-test-kitchen-enterprise-x86_64-linux"]
#   -> EXPEDITOR_PKG_IDENTS_CHEFTESTKITCHENENTERPRISEX86_64LINUX
PKG_VERSION="${EXPEDITOR_PKG_VERSION:-${EXPEDITOR_PROMOTABLE:-}}"
if [[ -z "$PKG_VERSION" && -n "${EXPEDITOR_PKG_IDENTS_CHEFTESTKITCHENENTERPRISEX86_64LINUX:-}" ]]; then
  PKG_VERSION=$(echo "${EXPEDITOR_PKG_IDENTS_CHEFTESTKITCHENENTERPRISEX86_64LINUX}" | cut -d'/' -f3)
fi
